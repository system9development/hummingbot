...
class BitrueOrderBookTrackerUnitTest(unittest.TestCase):
    order_book_tracker: Optional[BitrueOrderBookTracker] = None
    ...
    def setUpClass(cls):
        ...
        cls.order_book_tracker: BitrueOrderBookTracker = BitrueOrderBookTracker(cls.trading_pairs)
...
#                             eth_usdt.get_price(False))
    ...
    def test_tracker_integrity(self):
        ...
        self.ev_loop.run_until_complete(asyncio.sleep(10.0))
        order_books: Dict[str, OrderBook] = self.order_book_tracker.order_books
        ethbtc_book: OrderBook = order_books["ETH-USDT"]
        xrpusdt_book: OrderBook = order_books["XRP-USDT"]
        # print(btcusdt_book.snapshot)
        # print("xrpusdt")
        # print(xrpusdt_book.snapshot)
        self.assertGreaterEqual(ethbtc_book.get_price_for_volume(True, 10).result_price,
                                ethbtc_book.get_price(True))
        self.assertLessEqual(ethbtc_book.get_price_for_volume(False, 10).result_price,
    ...
    def test_api_get_last_traded_prices(self):
        prices = self.ev_loop.run_until_complete(
            self.order_book_tracker.data_source.get_last_traded_prices(["BTC-USDT", "LTC-BTC"]))
