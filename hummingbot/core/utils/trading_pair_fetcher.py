...
PROBIT_ENDPOINT = "https://api.probit.com/api/exchange/v1/market"
...
class TradingPairFetcher:
    ...
    @staticmethod
    async def fetch_probit_trading_pairs() -> List[str]:
        async with aiohttp.ClientSession() as client:
            async with client.get(PROBIT_ENDPOINT, timeout=API_CALL_TIMEOUT) as response:
                if response.status == 200:
                    from hummingbot.connector.exchange.probit.probit_utils import \
                        convert_from_exchange_trading_pair
                    try:
                        data: Dict[str, Any] = await response.json()
                        return [convert_from_exchange_trading_pair(item["id"]) for item in data["data"]]
                    except Exception:
                        pass
                        # Do nothing if the request fails -- there will be no autocomplete for kucoin trading pairs
                return []
    ...
    async def fetch_all(self):
        tasks = [self.fetch_binance_trading_pairs(),
                 ...
                 self.fetch_probit_trading_pairs()]
