...
from hummingbot.connector.exchange.bitrue.bitrue_order_book_tracker import BitrueOrderBookTracker
from hummingbot.connector.exchange.bitrue.bitrue_user_stream_tracker import BitrueUserStreamTracker
...
class BitrueMarket(ExchangeBase):
    ...
    @property
    def in_flight_orders(self) -> Dict[str, BitrueInFlightOrder]:
    ...
    def restore_tracking_states(self, saved_states: Dict[str, any]):
        ...
        self._in_flight_orders.update({
            key: BitrueInFlightOrder.from_json(value)
        ...
        while True:
            try:
                ...
                self.logger().network(f"Unexpected error while fetching trading rules. Error: {str(e)}",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Bitrue.com. "
    ...
    def _format_trading_rules(self, instruments_info: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        Response Example:
              ...
              'filters': [{'filterType': 'PRICE_FILTER',
                           'maxPrice': '0.0000068180000000000000000',
                           'minPrice': '0.0000003409000000000000000',
                           'priceScale': 9},
                          {'filterType': 'LOT_SIZE',
                           'maxQty': '99999999.0000000000000000',
            ...
            try:
                ...
                for _filter in rule['filters']:
                    if _filter['filterType'] == 'PRICE_FILTER':
                        # max_price = _filter['maxPrice']
                        # min_price = _filter['minPrice']
                        price_scale = _filter['priceScale']
                    if _filter['filterType'] == 'LOT_SIZE':
                        max_qty = _filter['maxQty']
                        min_qty = _filter['minQty']
                        volume_scale = _filter['volumeScale']

                supports_limit_orders = True if 'LIMIT' in rule['orderTypes'] else False
                supports_market_orders = True if 'MARKET' in rule['orderTypes'] else False
                price_step = Decimal("1") / Decimal(str(math.pow(10, price_scale)))
                quantity_step = Decimal("1") / Decimal(str(math.pow(10, volume_scale)))
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = BitrueInFlightOrder(
            client_order_id=order_id,
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str, wait_for_status: bool = False) -> str:
        ...
        try:
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg=f"Failed to cancel the order {order_id} on Bitrue. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.",
                                      exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Bitrue.com. "
